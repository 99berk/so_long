...
void	find_player_enemy_position(t_game *data)
{
	int	y;
	int	x;
...
int	player(t_game *data)
{
	...
	data->player_img[0] = mlx_xpm_file_to_image(data->mlx_ptr,
			"./images/player_img.xpm", &img_size, &img_size);
	...
	mlx_string_put(data->mlx_ptr, data->mlx_win, 1, 15, 0x00FF00, "Movecount:");
	mlx_string_put(data->mlx_ptr, data->mlx_win, 75, 15, 0x00FF00, move_str);
	free(move_str);
	usleep(100000);
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		img_uploud(&data);
		find_player_enemy_position(&data);
